...
def handle_expense(session, account, amount, device):
    ...
    if total_today > 20_000_000:
        ...
        for status in ["success", "failed"]:
            auth_log = create_auth_log(account.customer, device, "biometric", status, "authentication_over_20M")
    ...
    for idx, status in enumerate(auth_result):
        ...
        auth_logs.append(auth_txn)

    txn = create_transaction(account, device, amount, "expense", txn_status, auth_logs)
    for auth_txn in auth_logs:
        auth_txn.transaction = txn
        session.add(auth_txn.auth_log)
